import { a as event_handler_invalid, b as true_default, g as state_proxy_equality_mismatch, l as hydration_mismatch, n as await_waterfall, v as svelte_boundary_reset_noop } from "./warnings-Cy2Rahnz.js";
import { a as legacy_mode_flag, o as tracing_mode_flag, t as async_mode_flag } from "./flags-BwKXtxQs.js";
	...
	#traverse_effect_tree(root, target) {
		...
		while (effect$1 !== null) {
			...
			if (!skip && effect$1.fn !== null) {
				...
				else if (async_mode_flag && (flags$1 & (RENDER_EFFECT | MANAGED_EFFECT)) !== 0) target.render_effects.push(effect$1);
...
function flush_effects() {
	...
	try {
		...
		while (queued_root_effects.length > 0) {
			...
			old_values.clear();
			if (true_default) for (const source$1 of batch.current.keys())
 /** @type {Set<Source>} */ source_stacks.add(source$1);
		...
		last_scheduled_effect = null;
		if (true_default) for (const source$1 of source_stacks) source$1.updated = null;
	}
}
...
function derived(fn) {
	...
	if (true_default && tracing_mode_flag) signal.created = get_error("created at");
	return signal;
}
/**
* @template V
...
function update_derived(derived$1) {
	...
	if (batch_values !== null) {
		if (effect_tracking() || current_batch?.is_fork) batch_values.set(derived$1, value);
...
function source(v, stack$1) {
	...
	if (true_default && tracing_mode_flag) {
		signal.created = stack$1 ?? get_error("created at");
		signal.updated = null;
...
function mark_reactions(signal, status) {
	...
	for (var i = 0; i < length; i++) {
		...
		if ((flags$1 & DERIVED) !== 0) {
			...
			if ((flags$1 & BLOCK_EFFECT) !== 0 && eager_block_effects !== null) eager_block_effects.add(reaction);
			schedule_effect(reaction);
		}
	}
}
...
function proxy(value) {
	...
	var stack$1 = true_default && tracing_mode_flag ? get_error("created at") : null;
...
function destroy_effect(effect$1, remove_dom = true) {
	var removed = false;
	if ((remove_dom || (effect$1.f & HEAD_EFFECT) !== 0) && effect$1.nodes !== null && effect$1.nodes.end !== null) {
...
function update_effect(effect$1) {
	...
	try {
		if ((flags$1 & (BLOCK_EFFECT | MANAGED_EFFECT)) !== 0) destroy_block_effect_children(effect$1);
...
function get(signal) {
	...
	if (true_default) {
		...
		if (tracing_mode_flag && !untracking && tracing_expressions !== null && active_reaction !== null && tracing_expressions.reaction === active_reaction) if (signal.trace) signal.trace();
			...
			var trace$1 = get_error("traced at");
			if (trace$1) {
...
//#endregion
export { autofocus as $, TEMPLATE_FRAGMENT as $n, object_keys as $r, previous_batch as $t, deferred_template_effect as A, set_hydrate_node as An, DOCUMENT_FRAGMENT_NODE as Ar, async_derived as At, pause_effect as B, HMR as Bn, STATE_SYMBOL as Br, save as Bt, untrack as C, hydrate_next as Cn, snippet_without_render_tag as Cr, mutable_source as Ct, block as D, next as Dn, COMMENT_NODE as Dr, state as Dt, aborted as E, hydrating as En, BLOCK_EFFECT as Er, source as Et, effect_tracking as F, EACH_IS_ANIMATED as Fn, HEAD_EFFECT as Fr, capture as Ft, template_effect as G, NAMESPACE_HTML as Gn, fallback as Gr, pending as Gt, render_effect as H, HYDRATION_ERROR as Hn, array_from as Hr, unset_context as Ht, legacy_pre_effect as I, EACH_IS_CONTROLLED as In, INERT as Ir, flatten as It, validate_effect as J, PROPS_IS_BINDABLE as Jn, get_prototype_of as Jr, current_batch as Jt, user_effect as K, NAMESPACE_MATHML as Kn, get_descriptor as Kr, createSubscriber as Kt, legacy_pre_effect_reset as L, EACH_ITEM_IMMUTABLE as Ln, LEGACY_PROPS as Lr, for_await_track_reactivity_loss as Lt, eager_effect as M, skip_nodes as Mn, EFFECT_RAN as Mr, derived_safe_equal as Mt, effect as N, ATTACHMENT_KEY as Nn, EFFECT_TRANSPARENT as Nr, user_derived as Nt, branch as O, read_hydration_instruction as On, DESTROYED as Or, update as Ot, effect_root as P, EACH_INDEX_REACTIVE as Pn, ELEMENT_NODE as Pr, async_body as Pt, add_form_reset_listener as Q, PROPS_IS_UPDATED as Qn, noop as Qr, is_flushing_sync as Qt, managed as R, EACH_ITEM_REACTIVE as Rn, LOADING_ATTR_SYMBOL as Rr, run as Rt, tick as S, safe_not_equal as Sn, lifecycle_outside_component as Sr, internal_set as St, invalidate_inner_signals as T, hydrate_template as Tn, svelte_element_invalid_this_value as Tr, set as Tt, resume_effect as U, HYDRATION_START as Un, deferred as Ur, boundary as Ut, remove_effect_dom as V, HYDRATION_END as Vn, TEXT_NODE as Vr, track_reactivity_loss as Vt, teardown as W, HYDRATION_START_ELSE as Wn, define_property as Wr, get_boundary as Wt, listen_to_event_and_reset_event as X, PROPS_IS_LAZY_INITIAL as Xn, is_function as Xr, flushSync as Xt, listen as Y, PROPS_IS_IMMUTABLE as Yn, is_array as Yr, eager as Yt, without_reactive_context as Z, PROPS_IS_RUNES as Zn, is_promise as Zr, fork as Zt, safe_get as _, tag as _n, props_rest_readonly as _r, equals as _t, event as a, dev_current_component_function as an, TRANSITION_OUT as ar, create_comment as at, set_signal_status as b, snapshot as bn, invalid_default_snippet as br, proxy as bt, replay_events as c, getContext as cn, component_api_changed as cr, create_text as ct, active_reaction as d, pop as dn, get_abort_signal_outside_reaction as dr, get_next_sibling as dt, run$1 as ei, invoke_error_boundary as en, TEMPLATE_USE_IMPORT_NODE as er, remove_textarea_child as et, deep_read as f, push as fn, hydratable_missing_but_required as fr, init_operations as ft, is_destroying_effect as g, label as gn, props_invalid_value as gr, sibling as gt, get as h, get_error as hn, lifecycle_legacy_only as hr, should_defer_append as ht, delegate as i, createContext as in, TRANSITION_IN as ir, clear_text_content as it, destroy_effect as j, set_hydrating as jn, EFFECT_OFFSCREEN as jr, derived as jt, component_root as k, reset as kn, DIRTY as kr, update_pre as kt, root_event_handles as l, hasContext as ln, component_api_invalid_new as lr, first_child as lt, exclude_from_object as m, set_dev_current_component_function as mn, invalid_snippet as mr, set_attribute as mt, apply as n, to_array as ni, add_svelte_meta as nn, TEMPLATE_USE_SVG as nr, $window as nt, handle_event_propagation as o, dev_stack as on, UNINITIALIZED as or, create_element as ot, deep_read_state as p, setContext as pn, hydration_failed as pr, is_firefox as pt, user_pre_effect as q, NAMESPACE_SVG as qn, get_descriptors as qr, Batch as qt, create_event as r, component_context as rn, TRANSITION_GLOBAL as rr, child as rt, on as s, getAllContexts as sn, bind_invalid_checkbox_value as sr, create_fragment as st, all_registered_events as t, run_all as ti, queue_micro_task as tn, TEMPLATE_USE_MATHML as tr, $document as tt, active_effect as u, is_runes as un, each_key_duplicate as ur, get_first_child as ut, set_active_effect as v, tag_proxy as vn, rune_outside_svelte as vr, strict_equals as vt, update_version as w, hydrate_node as wn, store_invalid_shape as wr, mutate as wt, settled as x, dynamic_void_element_content as xn, invalid_snippet_arguments as xr, increment as xt, set_active_reaction as y, trace as yn, experimental_async_required as yr, is as yt, move_effect as z, FILENAME as zn, MAYBE_DIRTY as zr, run_after_blockers as zt };
//# sourceMappingURL=events-BumxY3U6.js.map